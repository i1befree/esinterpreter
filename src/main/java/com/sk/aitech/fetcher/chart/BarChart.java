...
/**
 * Created by i1befree on 15. 7. 16..
 */
@Fetcher(name="barchart", description="barchart(key=field1, value=field2, group=field3)")
public class BarChart extends ChartFetcher {
  final Pattern p = Pattern.compile("(barchart\\s*\\()(((\\w+\\s*\\=\\s*\\w+)(\\s*,\\s*)*)+)(\\))");

  /**
   * cmdLine like timechart(key=x, group=a, value=y)
   * @param context
   */
  public BarChart(FetcherContext context) throws FetcherException{
    super(context);
