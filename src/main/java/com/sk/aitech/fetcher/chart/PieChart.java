 ...
 * Piechart 용 명령어.
 */
@Fetcher(name="piechart", description = "piechart(key={keyfield}, group={groupfield}, value={valuefield})")
public class PieChart extends ChartFetcher {
  final Pattern p = Pattern.compile("(piechart\\s*\\()(((\\w+\\s*\\=\\s*\\w+)(\\s*,\\s*)*)+)(\\))");

  /**
   * cmdLine like timechart(key=x, group=a, value=y)
   * @param context
   */
  public PieChart(FetcherContext context) throws FetcherException{
    super(context);
